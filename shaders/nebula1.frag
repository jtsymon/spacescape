...
void main(void) {
    /*
    float field = seed;
    float divisor = 2.f / (2 << 5);
    for(int x = 5; x > 0; x--) {
        field = snoise(vec2(abs(UV.x) / divisor + field, abs(UV.y) / divisor + field));
        divisor *= 2;
    }
    ...
    */
    ...
    // Cloudy:
    float i = min(1, snoise(UV + seed.x) * intensity + snoise(UV * seed.y) * 0.1 + snoise(vec2(seed.z, seed.s)) * 0.1);
    // i = snoise(UV * i);
